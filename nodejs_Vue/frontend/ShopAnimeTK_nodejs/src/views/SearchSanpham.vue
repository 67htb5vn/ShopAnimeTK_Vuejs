...
const mahh = computed(() => route.query.MaHh as string || '')
const search = computed(() => route.query.search as string || '')
const tenhh = ref<string>('');
const soluong = ref<number>(0);
...
const filters = ref({
    ...
    mahh: (route.query.MaHh as string) ?? '',
...
const loadSanphamDmh = async () => {
    try {
        ...
        if (sps.value.length > 0) {
            tenhh.value = sps.value[0]?.tenhh || 'Hoạt hình';
            soluong.value = sps.value.length;;
        }
    } catch (error) {
...
watch(
    ...
    () => {
        ...
        filters.value.mahh = (route.query.MaHh as string) ?? '';
        ...
        <div class="container">
            <nav aria-label="breadcrumb" class="breadcrumb-nav">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item">
                        <router-link :to="`/Hoathinh?MaHh=${mahh}`">
                            {{ tenhh }}
                        </router-link>
                    </li>
                </ol>
            ...
            <div class="row">
                <div class="col-lg-9 main-content">
                    <h4 class="account-sub-title d-none d-md-block mb-4">
                        Kết quả tìm kiếm của "{{ search }}" ( {{ soluong }} kết quả )
                    </h4>
